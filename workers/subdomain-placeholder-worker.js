...
async function renderLog(requestUrl) {
  ...
  const filterButtons = [`<button class="filter-button" type="button" data-filter="all" aria-pressed="true">All notes</button>`, ...types.map((type) => `<button class="filter-button" type="button" data-filter="${escapeHtml(type)}" aria-pressed="false">${escapeHtml(type)}</button>`)].join("");
  const cards = posts.map((post, index) => `<a class="note-card${index === 0 ? " note-card--feature" : ""}" data-note-card data-note-type="${escapeHtml(post.type)}" href="/post/${escapeHtml(post.slug)}"><div><div class="note-card__tag meta">${escapeHtml(post.date)} / ${escapeHtml(post.type)}</div><h2>${inlineMarkdown(post.title)}</h2><p>${inlineMarkdown(post.excerpt)}</p></div><div class="note-card__foot meta"><span>${escapeHtml(post.readTime)}</span><span>Read note →</span></div></a>`).join("");
  const content = `<h1>Field <em>log.</em></h1><p class="lede">A public working archive of systems, experiments, and the decisions that shape them. Every note starts as Markdown in the portfolio repository.</p><div class="actions"><a class="button button--solid" href="https://github.com/hsndm566/hsndm.me/tree/main/notes" target="_blank" rel="noreferrer">Open source archive ↗</a><a class="button" href="https://hsndm.me">Return to portfolio →</a></div><hr class="rule"><div class="filter-bar" aria-label="Filter build notes">${filterButtons}</div><div class="log-grid">${cards || '<div class="empty">The first note is being prepared.</div>'}</div>`;
  const script = `<script src="https://cdn.jsdelivr.net/npm/animejs@3.2.2/lib/anime.min.js" defer></script><script defer>const reduced=window.matchMedia("(prefers-reduced-motion: reduce)").matches;const buttons=[...document.querySelectorAll("[data-filter]")];const cards=[...document.querySelectorAll("[data-note-card]")];const applyFilter=(filter)=>{buttons.forEach((button)=>button.setAttribute("aria-pressed",String(button.dataset.filter===filter)));const visible=cards.filter((card)=>filter==="all"||card.dataset.noteType===filter);cards.forEach((card)=>{card.style.display=visible.includes(card)?"flex":"none"});if(!reduced&&window.anime)window.anime({targets:visible,opacity:[0,1],translateY:[12,0],delay:window.anime.stagger(65),duration:460,easing:"easeOutExpo"})};buttons.forEach((button)=>button.addEventListener("click",()=>applyFilter(button.dataset.filter)));</script>`;
  return response(shell({ title: "Field log", eyebrow: "Build archive / public notes", host: "log.hsndm.me", content, script, noIndex: false }));
}
...
async function renderStatus(requestUrl) {
  ...
  const list = statuses.map((service) => `<div class="service${service.ok ? "" : " service--issue"}" data-service-row><span class="service__dot"></span><div><div class="service__name">${escapeHtml(service.name)}</div><div class="service__detail">${escapeHtml(service.detail)}</div></div><div class="service__metric">${escapeHtml(service.code)} · ${escapeHtml(service.latency)} ms</div></div>`).join("");
  ...
  const script = `<script src="https://cdn.jsdelivr.net/npm/animejs@3.2.2/lib/anime.min.js" defer></script><script defer>let remaining=60;const target=document.getElementById("countdown");const reduced=window.matchMedia("(prefers-reduced-motion: reduce)").matches;const refreshPulse=()=>{if(reduced||!window.anime)return;window.anime({targets:".status-summary",translateY:[0,-3,0],duration:420,easing:"easeOutQuad"});window.anime({targets:"[data-service-row]",opacity:[1,.52,1],translateX:[0,5,0],delay:window.anime.stagger(75),duration:520,easing:"easeOutQuad"})};setInterval(()=>{remaining-=1;if(target)target.textContent=remaining;if(remaining===3)refreshPulse();if(remaining<=0)location.reload()},1000)</script>`;
